SELECT TOP ENTER_TOP_COUNT
  qp.query_id,
--  qp.plan_id,
  FORMAT(MAX(rs.last_execution_time) AT TIME ZONE 'ENTER_TIME_ZONE', 'ENTER_DATE_FORMAT') AS LastExecution,
...
--  ROUND(SUM(rs.avg_physical_io_reads * rs.count_executions)/SUM(rs.count_executions),3) AS AvgPhysicalReads,
  MIN(rs.min_physical_io_reads) AS MinAvgPhysicalReads,
  MAX(rs.max_physical_io_reads) AS MaxAvgPhysicalReads,  
